...
## Phase 6: the audit log is written by a database trigger, not by server actions

`audit_row_change()` (migration `0013`) is a `security definer` after-trigger on every business table (vehicles, images, catalog, shipping, inquiries, quotes, orders, payments, shipments, documents, profiles, site_settings, cms_pages). It records `auth.uid()` as the actor, the operation, and the full old/new row as JSON. A trigger was chosen over logging in each action because RLS lets several roles write some tables straight over REST, and an action that forgets to log would silently leave a gap. Updates where only `updated_at` changed are skipped. `audit_logs` is append-only: admins can `select`, and insert/update/delete/truncate are revoked from `anon` and `authenticated`, so not even an admin JWT can rewrite history. Changes made with the service role or `psql` (seeding, migrations) are logged with a null actor ("System"). Favorites and saved searches aren't audited, since they're personal client data with no business consequence. The log holds full rows, including client contact data and private chassis/VIN numbers, which is why it's admin-only.

## Phase 6: dashboard metrics come from one security-definer RPC

`dashboard_metrics()` aggregates the spec §4.1 KPIs in SQL and returns JSON, instead of a dozen PostgREST count queries. It checks `get_my_role()` itself: staff get stock figures, only admin/sales get the commerce figures (leads, quotes, reservations, pending payments, verified revenue, top makes/countries), and anyone else gets `42501`. Execute is revoked from `public`/`anon`. "Leads today" uses the Japan business day (`Asia/Tokyo`). "Top countries" uses the ordering client's consignee country, falling back to their profile country, because orders don't store a destination country of their own. Revenue is the sum of verified payments.

## Phase 6: CMS content lives in `site_settings` keys and `cms_pages`, validated by zod on read and write

Spec §4.4's editable content is split in two. Structured blocks are `site_settings` keys (`alert_banner`, `hero`, `contact`, `faq`, `testimonials`, plus Phase 5's `bank_details`), each with a zod schema in `src/lib/settings/schema.ts`. Writes are validated in `src/actions/cms.ts`. Reads go through `parseSetting()`, which merges the stored JSON over defaults and falls back to the defaults if the row is invalid, so a bad row can never break a public page. Long-form pages (About, How to Buy, Shipping, Contact) are `cms_pages` rows whose `content_json.body` is plain text with a tiny markup subset: `## ` headings, `- ` bullets and blank-line paragraphs. `parseContentBlocks()` turns that into React elements, so no HTML is ever interpreted and admin-entered content can't inject script or markup. This was chosen over a rich-text/Markdown library to avoid adding an HTML sanitizer dependency. Links the admin types (hero button, banner link) must be site-relative (`/…`, not `//…`) or `https://`. Social links must be `https://`. "Countries served" isn't a separate setting: the home and shipping pages list the countries that have at least one active port, so the Shipping admin stays the single source of truth. Testimonials start empty and the section stays hidden until the owner adds real ones, so no reviews are invented. Only the `en` locale exists; i18n is still out of scope.

## Phase 6: promotions reuse `vehicles.featured` and `sale_price_usd`

Both columns existed since Phase 0/2. A vehicle is "on promotion" when it's featured or has a sale price. `/admin/promotions` (admin + inventory_manager, same as `vehicles_write_staff`) toggles `featured` and sets or clears the sale price, and refuses a sale price that isn't below the regular price. The home page shows a "Promotions & featured" section, and `/stock` gained the spec §3.2 "Promotion" filter (`promotion=1`). Both use a constant PostgREST `.or()` string, never user input. There are no promotion start/end dates or campaign records; that needs a scheduler this project still doesn't have.
